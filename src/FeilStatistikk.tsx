...
const FeilStatistikk = () => {
    ...
    const { items, requestSort, sortConfig } = useTableSorting<StatistikkInfo>(statistikkInfoList ? statistikkInfoList.map(item => {
        if (typeof item.antall_feil === 'string') item.antall_feil = parseInt(item.antall_feil);
        return item;
    }) : null);
