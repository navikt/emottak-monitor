...
const EventsTable = (props) => {
  ...
  function filterRole(selectedRole) {
    ...
    setVisibleEvents([
      ...events.filter(function (EventDetails) {
        return (
          (selectedRole === "" || EventDetails.role === selectedRole) &&
  ...
  function filterService(selectedService) {
    ...
    setVisibleEvents([
      ...events.filter(function (EventDetails) {
        return (
          (role === "" || EventDetails.role === role) &&
          (selectedService === "" ||
            EventDetails.service === selectedService) &&
          (action === "" || EventDetails.action === action) &&
          (status === "" || EventDetails.status === status)
        );
      }),
  ...
  const applyFilter = useCallback(
    (newEvents) => {
      setVisibleEvents([
        ...newEvents.filter(function (EventDetails) {
          return (
            (role === "" || EventDetails.role === role) &&
            (service === "" || EventDetails.service === service) &&
            (action === "" || EventDetails.action === action) &&
  ...
  const pushHistory = useCallback(() => {
    history.push(
      `/events?fromDate=${fom}&fromTime=${fromTime}&toDate=${tom}&toTime=${toTime}&role=${role}&service=${service}&action=${action}&status=${status}`
    );
            ...
            return (
              <tr>
                <td className="tabell__td--sortert">
                  {EventDetails.hendelsedato.substr(0, 23)}
                </td>
                <td>
                ...
                <td>
                  <Lenke href={`/logg/${EventDetails.mottakid}`}>
                    {EventDetails.mottakid}{" "}
                  </Lenke>
                </td>
